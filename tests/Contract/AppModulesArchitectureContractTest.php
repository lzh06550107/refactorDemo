...
(static function (): void {
    ...
    if (is_dir($moduleRoot)) {
        ...
        foreach ($iterator as $sourceFile) {
            if (!$sourceFile instanceof SplFileInfo || !$sourceFile->isFile() || strtolower($sourceFile->getExtension()) !== 'php') {
                continue;
            }
    ...
    foreach ([$root . '/app', $root . '/modules', $root . '/config', $root . '/tests'] as $activeRoot) {
        ...
        foreach ($iterator as $sourceFile) {
            ...
            if ($sourceFile->getFilename() === 'AppModulesArchitectureContractTest.php') {
                continue;
            }
            ...
            foreach ($staleModules as $module) {
                ...
                expectTrue(
                    !str_contains($source, $prefix),
                    'stale business namespace remains: ' . $sourceFile->getPathname() . ' -> ' . $prefix,
                );
            }
        }
    }
