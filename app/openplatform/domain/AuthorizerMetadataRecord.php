...
final readonly class AuthorizerMetadataRecord
{
    ...
    private function __construct(
        ...
        private DateTimeImmutable $providerFetchedAt,
        private int $version,
    ) {
        if (trim($componentPlatformId) === '' || trim($authorizerAppId) === '') {
            throw new InvalidArgumentException('Authorizer metadata record identity must not be empty.');
        }
        if (!preg_match('/^[a-f0-9]{64}$/', $metadataHash)) {
            throw new InvalidArgumentException('Authorizer metadata record hash must be a lowercase SHA-256 hex digest.');
        }
        if (trim($normalizedMetadataJson) === '') {
            throw new InvalidArgumentException('Normalized authorizer metadata JSON must not be empty.');
        }
        if ($version < 1) {
            throw new InvalidArgumentException('Authorizer metadata record version must be positive.');
        }
    }

    public static function fromMetadata(
        AuthorizerMetadata $metadata,
        DateTimeImmutable $providerFetchedAt,
        int $version,
    ): self {
        return new self(
            $metadata->componentPlatformId(),
            $metadata->authorizerAppId(),
            $metadata->accountType(),
            $metadata->nickName(),
            $metadata->headImageUrl(),
            $metadata->originalId(),
    ...
    public function providerFetchedAt(): DateTimeImmutable { return $this->providerFetchedAt; }
