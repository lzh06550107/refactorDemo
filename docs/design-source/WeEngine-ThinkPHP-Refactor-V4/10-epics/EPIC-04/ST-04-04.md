# ST-04-04 — TenantModule/AccountModuleConfig

所属：EPIC-04 Module Platform

## User Story
作为平台/租户相关角色，我希望系统提供“TenantModule/AccountModuleConfig”，以在保持 WeEngine 核心业务结果兼容的同时获得更清晰、可测试的实现。
...
### 关键业务规则
`TenantModule` 表达租户/商业授权；`AccountModuleConfig` 表达账号级运行配置；不得用用户权限代替。`module_fetch()` 的账号 overlay 迁移到 RuntimeModuleResolver，但 Effective Access 仍由 Entitlement + Authorization 联合决定。
